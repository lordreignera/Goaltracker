...
class UpdateGoalRequest extends FormRequest
{
    public function authorize(): bool
    {
        ...
        return $user && $goal && app(GoalAccessService::class)->canUpdateGoal($user, $goal);
    }
    ...
    public function after(): array
    {
        return [
            ...
            function (Validator $validator) {
                ...
                if (! $goal || $objectiveIds->isEmpty()) {
                    return;
                }

                $validCount = $goal->objectives()
                    ->whereIn('id', $objectiveIds->all())
                    ->count();

                if ($validCount !== $objectiveIds->count()) {
                    $validator->errors()->add('objectives', 'One or more objectives do not belong to this goal.');
                }
            },
            function (Validator $validator) {
